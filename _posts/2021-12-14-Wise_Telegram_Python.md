...
**/tw_on**  _AUD-USD-Target_ - check exchange rate every 600 seconds - if checked exchange rate is better or equal to the target exchange rate then Send Message

**/tw_off** - turn off currency checks

**/tw_now** - currency checks now

The Animated GIF below has been edited to be fast forward 10 mins ahead, from "/tw_on .." command to show Python responding with a currency check, since the currency price is better than targeted one.
